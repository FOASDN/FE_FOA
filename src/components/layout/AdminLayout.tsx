...
const SidebarItem = ({ icon, label, href, isActive, isCollapsed, currentPath, subItems }: SidebarItemProps) => {
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className={cn(
            ...
            (isActive || hasActiveChild) ? "text-orange-500 scale-105" : "text-[#9a734c]/60 dark:text-gray-500 group-hover:text-orange-500 group-hover:scale-105"
          )}>
            {icon}
          </span>
      ...
      >
          ...
          >
            ...
            {isHovered && (
              <div className="overflow-hidden whitespace-nowrap animate-in fade-in duration-200">
                <h1 className="text-xl font-black tracking-tighter leading-none">FoodieDash</h1>
                <p className="text-[9px] text-orange-600/70 uppercase tracking-wider font-bold mt-0.5">Khu vực quản trị</p>
              </div>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-0.5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {NAV_ITEMS.map((item) => {
            ...
            const isExactMatch = item.href === "/admin/overview"
